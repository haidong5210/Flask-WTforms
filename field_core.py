...
class Field(object):
    ...
    def __str__(self):
        ...
        see the `__call__` method.
        """
        return self()      #此处是在对象后面加（）执行自己的__call__方法
        ...
        if formdata:
            try:
                if self.name in formdata:
                    ...
                    self.raw_data = []
                self.process_formdata(self.raw_data)  #这里执行字段对象的process_formdata
            except ValueError as e:
                self.process_errors.append(e.args[0])
...
class UnboundField(object):
    ...
    def __init__(self, field_class, *args, **kwargs):     #field_class=对应字段的的类名
...
class StringField(Field):
    ...
    def process_formdata(self, valuelist):
        if valuelist:
            self.data = valuelist[0]   # data存储的值
        else:
            self.data = ''
