...
class BaseForm(object):
    ...
    def __init__(self, fields, prefix='', meta=DefaultMeta()):   #fields = self._unbound_fields
        """
        :param fields:
        ...
        """
        if prefix and prefix[-1] not in '-_;:/.':   #name属性前面的前缀
            prefix += '-'

        self.meta = meta    
        self._prefix = prefix
        self._errors = None
        self._fields = OrderedDict()    # _fields = {'name':StringField(),'pwd':PasswordField()}
        
        if hasattr(fields, 'items'):
            fields = fields.items()

        translations = self._get_translations()      #错误信息显示的文字类型
        extra_fields = []
        if meta.csrf:    #显示隐藏的那个csrf
            self._csrf = meta.build_csrf(self)    
            extra_fields.extend(self._csrf.setup_form(self))

        for name, unbound_field in itertools.chain(fields, extra_fields):
            options = dict(name=name, prefix=prefix, translations=translations)
            field = meta.bind_field(self, unbound_field, options)  #此处返回的是对应字段的实例化对象 例如：StringField()
            self._fields[name] = field  
            #_fields = {'name':StringField(),'pwd':PasswordField()}
    ...
    def process(self, formdata=None, obj=None, data=None, **kwargs):
        ...
        for name, field, in iteritems(self._fields):     #field对应的是各自的字段对象
            if obj is not None and hasattr(obj, name):
                field.process(formdata, getattr(obj, name)) 
                #执行对应字段的process方法，最后赋值给对应字段的data静态属性里
                #取指可以这样:    form.对应字段名.data
            elif name in kwargs:
...
class FormMeta(type):
    ...
    """
    def __init__(cls, name, bases, attrs):  #name = NewBase bases= (BaseForm,) attrs={}
        type.__init__(cls, name, bases, attrs)
        cls._unbound_fields = None    # [(name,UnboundField()),(pwd,UnboundField()).........]
        cls._wtforms_meta = None      # 继承了所有Meta的类
    ...
    def __call__(cls, *args, **kwargs):
        ...
        if cls._unbound_fields is None:
            fields = []
            for name in dir(cls):  #cls 自定义的那个login
                if not name.startswith('_'):
                    unbound_field = getattr(cls, name)
                    if hasattr(unbound_field, '_formfield'):
                        fields.append((name, unbound_field))    #(name,UnboundField())
            # We keep the name as the second element of the sort
            # to ensure a stable sort.
            fields.sort(key=lambda x: (x[1].creation_counter, x[0])) # 通过UnboundField()的creation_counter静态属性排序
...
class Form(with_metaclass(FormMeta, BaseForm)): #这种形式相当于Form(metaclass=FormMeta(BaseForm)
    '''
    with_metaclass(FormMeta, base=BaseForm)
    return FormMeta("NewBase", (BaseForm,), {})
    实例化Form，第一步实例化了FormMeta("NewBase", (BaseForm,), {})
    第二步实例化Form，第一执行FormMeta的__call__方法，执行Form的__init__
    ...
    def __init__(self, formdata=None, obj=None, prefix='', data=None, meta=None, **kwargs):
        ...
        :param `**kwargs`:
            ...
            of a matching keyword argument to the field, if one exists.
        """
        meta_obj = self._wtforms_meta()    #实例化继承了所有Meta的类
        ...
        for name, field in iteritems(self._fields):   
            ...
            setattr(self, name, field)  #给Form实例化的对象赋值 例如：name = StringField()， pwd=PasswordField()
        self.process(formdata, obj, data=data, **kwargs)  
        '''
        self.process自己没有找基类，在BaseForm的process
        formdata，obj，data参数都是在前端显示默认值的
        formdata对应的值必须有getlist方法才阔以，例如request.args
        obj=对象
        data = {'name':'haidong'}
        '''
